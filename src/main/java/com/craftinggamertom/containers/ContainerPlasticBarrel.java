...
import com.craftinggamertom.slots.PBarrelBucketSlot;
...
public class ContainerPlasticBarrel extends Container{
	...
	public ContainerPlasticBarrel(InventoryPlayer playerInventory, PBTileEntity te)
	{
		...
		*/
		...
		//New Version
		this.addSlotToContainer(new PBarrelCocaSlot(te, 0, 56, 17));
		this.addSlotToContainer(new PBarrelWaterSlot(te, 1, 56, 53));
		this.addSlotToContainer(new PBarrelPasteSlot(te, 2, 112, 35));
		this.addSlotToContainer(new PBarrelBucketSlot(te, 3, 130, 35));
...
It's possible to do more complicated merges, but this is enough for now.
	...
	public ItemStack transferStackInSlot(EntityPlayer playerIn, int fromSlot) {
		 ...
		 slot = (Slot) this.inventorySlots.get(fromSlot);
		    if (slot != null && slot.getHasStack()) {
		        ...
		        // [...] Custom behavior
		        if (fromSlot < 4) // Set to the number of Tile Entity Slots
		        {
		            // From TE Inventory to Player Inventory
		            if (!this.mergeItemStack(current, 4, 40, true)) // set to (current, Number of Title Entity Slots, Number where Inventory starts) -usually 39
		                return null;
		        } else {
		            // From Player Inventory to TE Inventory
		            if (!this.mergeItemStack(current, 0, 4, false)) // set to (current, start of title entity index, last tile entity index)
		                return null;
		        }
		        // [...] End Custom behavior
    ...
    public void addCraftingToCrafters(ICrafting listener)
    {
            ...
            listener.sendProgressBarUpdate(this, 0, te.currentItemBurnTime);
    ...
    public void detectAndSendChanges()
    {
        ...
        for (int i = 0; i < crafters.size(); ++i)
        {
            ...
            if (currentItemBurnTime != te.currentItemBurnTime)
            {
                icrafting.sendProgressBarUpdate(this, 1, te.currentItemBurnTime);
            }
