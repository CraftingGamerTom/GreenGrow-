...
import com.craftinggamertom.block.DryBench;
...
public class DTTileEntity extends TileEntity implements ISidedInventory {
	...
	public void updateEntity() {
		...
		if (wasOn != isOn) {
			...
			// being used
			// this.updateDTBlockState(this.timeDrying > 0, this.worldObj,
	...
	private boolean dryable() {
		...
		for (int i = 1; i < this.inventory.length; i++) {
			...
			if (!theItemToCheck.equals(MItems.GreenCannabis)
					&& !theItemToCheck.equals(MItems.OrangeCannabis)
					&& !theItemToCheck.equals(MItems.PurpleCannabis)
					&& !theItemToCheck.equals(MItems.Shrooms)) {
				return false;
			}
		}
		return true;
	...
	private boolean readyToDry() {
		...
		/*
        if (this.inventory[0] == null) return true; // Checks if the product slot is empty.
        if (!this.inventory[0].isItemEqual(itemstack)) return false; // Checks if the item is the same in the product slot.
        int result = inventory[0].stackSize + itemstack.stackSize; // Gets the resulting stacksize after being dried to make sure it is legal.
	...
	private void dry(Item itemType) {
		...
		ItemStack theProduct = new ItemStack(product, 1);
		for (int i = 1; i < inventory.length; i++) {
			inventory[i] = theProduct.copy();
		}
		this.markDirty();
	}
	...
	public String getName() {
		return this.hasCustomName() ? this.customName : "container.drytable";
	}
